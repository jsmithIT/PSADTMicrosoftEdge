...
Try {
	...
	[string]$appVersion = '102.0.1245.41'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.2.0'
	[string]$appScriptDate = '2022.06.14'
	[string]$appScriptAuthor = 'KTEUTON, JBSMITH'
	...
	$CloseApps = "iexplore=Internet Explorer,msedge=Microsoft Edge"
	...
	$32bitAppInstallName = "Microsoft Edge" 
	# Application install Path.
	$32bitAppInstallPath = "MicrosoftEdgeEnterpriseX64.msi"
	# 32-bit application transform.
	#$32bitAppInstallTransform = ""
	# 32-bit application patch. 
	#$32bitAppInstallPatch = ""
	# Application install parameters.
	$32bitAppInstallParam = "REBOOT=ReallySuppress /QN"
	...
	$RemoveAppNamesMSI = @("Microsoft Edge")
	#$RemoveAppNamesEXE = @("")
	...
	$32bitAppUninstallName = "Microsoft Edge" 
	# Application uninstall path.
	...
	# Shortcuts to be copied to start menu.
	#[string[]]$appShortcuts = @("Acrobat Reader 2017.lnk")
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$RemoveAppNamesMSICheckVersion = Get-InstalledApplication -Name "$32bitAppUninstallName" -Exact | Select-Object DisplayVersion -expand DisplayVersion
		Write-Log -Message "$envOSVersionMajor"
		...
		If ($RemoveAppNamesMSICheckVersion -le $appVersion) {
			Try {
				...
				Execute-MSI -Action Install -Path "$dirFiles\$32bitAppInstallPath" -Parameters "$32bitAppInstallParam"
